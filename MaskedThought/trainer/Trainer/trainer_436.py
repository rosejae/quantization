...
from transformers.trainer_callback import TrainerControl, ExportableState
...
#        print(vars(args))
        ...
        self.processing_class= auto_tokenizer
        #self.add_callback(TrainerControl())
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if args.local_rank <= 0:
                    ...
                    if (step + 1) % print_every == 0:
                        self.his_info.lr = self.optimizer.param_groups[0]['lr']
                        self.his_info.update()
                        self.his_info.print_info(tokenizer=self.processing_class, step=step)
                ...
                if (
                    ...
                    self._maybe_log_save_evaluate(tr_loss=tr_loss, grad_norm=None, model=self.model, trial=trial, epoch=epoch, ignore_keys_for_eval= ignore_keys_for_eval)
                else:
    ...
    def compute_loss(self, model, inputs, return_outputs=False,**kwargs):
        ...
        if num_items_in_batch is not None:
            logger.info(f"num_items_in_batch received: {num_items_in_batch}")
        ...
        if isinstance(model, torch.nn.DataParallel) or isinstance(model, torch.nn.parallel.DistributedDataParallel):
            model.module.tokenizer = self.processing_class
            model.tokenizer = self.processing_class
        else:
